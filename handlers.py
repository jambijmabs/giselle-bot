...
def rephrase_gerente_response(answer: str, client_name: str, question: str, message_handler: Any) -> str:
    """Rephrase a gerente's response for a friendlier tone.

    Args:
        answer (str): The gerente's response.
        client_name (str): The client's name.
        question (str): The client's question.
        message_handler (Any): The message handler instance.

    Returns:
        str: The rephrased response.
    """
    prompt = (
...
def determine_best_contact_time(state: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
    """Determine the best contact time based on client interaction history.

    Args:
        state (dict): The client state dictionary.

    Returns:
        tuple: A tuple of (best_time, best_day) or (None, None) if no data.
    """
...
def handle_client_message(
    ...
    message_handler: Any,
    utils: Any,
    recontact_handler: Any
) -> Tuple[str, int]:
    """Handles incoming messages from clients.

    Args:
        phone (str): The client's phone number.
        incoming_msg (str): The incoming message text.
        num_media (int): The number of media attachments.
        media_url (Optional[str]): The URL of the media attachment, if any.
        profile_name (Optional[str]): The client's profile name, if available.
        conversation_state (dict): The current conversation state.
        client (Any): The Twilio client instance.
        message_handler (Any): The message handler instance.
        utils (Any): The utility functions instance.
        recontact_handler (Any): The recontact handler instance.

    Returns:
        tuple: A tuple of the form (message, status_code), where message is a 
        string describing the result of the operation, and status_code is an 
    ...
    try:
        ...
        if not state:
            raise KeyError(f"No state found for phone {phone}")
        ...
        logger.error(f"Error accessing conversation state for {phone}: {str(e)}", exc_info=True)
        error_message = f"Lo siento, {state.get('client_name', 'Cliente')}, ocurrió un error al procesar tu mensaje. Por favor, intenta de nuevo más tarde."
        utils.send_consecutive_messages(phone, [error_message], client, bot_config.WHATSAPP_SENDER_NUMBER)
        return "Error in conversation state", 500
    except Exception as e:
        logger.error(f"Unexpected error in handle_client_message for {phone}: {str(e)}", exc_info=True)
        error_message = f"Lo siento, {state.get('client_name', 'Cliente')}, ocurrió un error inesperado al procesar tu mensaje. Por favor, intenta de nuevo más tarde."
        utils.send_consecutive_messages(phone, [error_message], client, bot_config.WHATSAPP_SENDER_NUMBER)
